...
async function processPendingOrders() {
    ...
    try {
        ...
        for (const item of watchlistTickers)
            tickers.add(item.id_stock);
        for (const item of portfolioTickers)
            tickers.add(item.id_stock);
        const quotes = await (0, finnhub_1.fetchQuotesForTickers)([...tickers]);
        for (const order of pendingOrders) {
            const currentPrice = quotes.get(order.id_stock);
            if (!currentPrice || currentPrice <= 0) {
                continue;
            }
            const executionPrice = new client_1.Prisma.Decimal(currentPrice.toFixed(6));
            if (order.tipo === 'Buy') {
                if (!order.importo_investito)
                    continue;
                const quantity = order.importo_investito.div(executionPrice);
                await prisma_1.prisma.transazione.updateMany({
                    where: {
